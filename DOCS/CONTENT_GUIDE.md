...
- **Inspirations** (`inspirations.html`) [AMENDED 2026-06-10]: **Informative directory** — factual third-person blurbs; no catchphrases, emoji stats, gradient banners, or CTA buttons. Read the **AGENT NOTE** HTML comment at top of the file. Layout: `.inspirations-sections` responsive grid (1 → 2 → 3 columns via container queries on `main.content`) wrapping `.inspiration-section` cards; each section has `h2` + `.inspiration-list` entry grid (`auto-fit` when wide). **A–Z within each section**. Pick section: Game development · Software engineering · AI & machine learning · Hardware & PC · News & commentary · Culture & long-form. Also add **`js/search-data.js`** (`category: page`).
